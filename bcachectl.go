...
func (b *bcache_devs) RunTune(device string, tunable string) {
  if device == "" {
    ...
    return
  } else if x := b.IsBCDevice(device); x == false {
    fmt.Println(device, "does not appear to be a valid bcache device.\n") 
    return
  }
...
func parseArgs(all_args []string, allflags *flags) (cmds []string, help bool) {
  ...
  //already_formatted, _ := regexp.MatchString("Already a bcache device", out)
    if is_flag, _ := regexp.MatchString("^-.*", all_args[i]); is_flag {
      if all_args[i] == "-h" {
        help = true 
      } else if all_args[i] == "-f" {
        if checkFormat(all_args[i+1]) {
          allflags.Format = all_args[i+1]
          i++
        }
      } else if all_args[i] == "--wipe-bcache" {
        allflags.Wipe = true
      }
    } else {
        cmds = append(cmds, all_args[i])
    }
  }
  for j:=0; j<2; j++  {
    cmds = append(cmds, "")
